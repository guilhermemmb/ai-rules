...
def main():
    ...
    if args.sync_agents:
        ...
        if not config:
            ...
            return 1
        if not patch_settings(str(settings_file), config, logger, agents_dir=str(subagents_dir)):
    ...
    # Step 3: Distribute rules via rulesync (deploys subagents + main to ~/.claude/agents/)
    logger.info("Step 3: Distributing rules via rulesync")
