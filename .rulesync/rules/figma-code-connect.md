...
If anything looks uncertain, consult the [official Code Connect docs](https://help.figma.com/hc/en-us/articles/23920389749655-Code-Connect).
...
## Steering Workflow: Code Connect Components

This steering workflow helps connect Figma design components to their corresponding code implementations.

### Step 1: Get Code Connect Suggestions

Call `get_code_connect_suggestions` to identify all unmapped components:
...
3. Compare the code component's props with the Figma component properties
4. Identify the best match based on structural similarity
5. If multiple candidates are equally good, pick the one with the closest prop-interface match

### Step 3: Present Matches to User

Present findings and let the user choose which mappings to create:
...
Would you like to connect these components?
```

### Step 4: Create Code Connect Mappings

Once confirmed, call `send_code_connect_mappings`:
...
**Valid labels:** 'React', 'Web Components', 'Vue', 'Svelte', 'Storybook', 'Javascript', 'Swift UIKit', 'Objective-C UIKit', 'SwiftUI', 'Compose', 'Java', 'Kotlin', 'Android XML Layout', 'Flutter', 'Markdown'
