...
**NEVER** run any `gh` CLI command or GitHub API call that writes, mutates, or
modifies state. This is NOT overridable.

**Blocked:** `gh pr create/merge/close/edit/comment/review` ·
...
**Allowed:** `gh pr list/view/diff/checks` · `gh issue list/view` ·
`gh run list/view` · `gh workflow list/view` · `gh api` (GET only) ·
`gh auth token/status`
...
2. **Description**: use `.github/PULL_REQUEST_TEMPLATE.md` if present. Include
   before/after metrics table when applicable (lint counts, bundle size, test
   scores).
...
4. **Show only** in chat: file path, one-line summary, and the exact `gh`
   command to run.
