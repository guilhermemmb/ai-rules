...
- `search_graph` — find symbols by name, query, or semantic. Find exact
  qualified_name before reading source.
...
- `get_architecture` — understand package/service layout before cross-cutting
  changes

Prefer these over grep/glob for discovering context. Fall back to Read only for
